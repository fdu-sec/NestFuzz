...
// unknown. The other supported annotations for uninstrumented functions are// "functional" and "discard", which are described below under
// DataFlowSanitizer::WrapperKind.
// Functions will often be labelled with both "uninstrumented" and one of
// "functional" or "discard". This will leave the function unchanged by this// pass, and create a wrapper function that will call the original.
//
...
static cl::list<std::string> ClCombineTaintLookupTables(
    ...
    cl::desc(
        ...
        "loading specific constant global variables (i.e. lookup tables)."),    cl::Hidden);
...
// Currently callbacks are only inserted for loads, stores, memory transfers// (i.e. memcpy and memmove), and comparisons.
//
...
// Experimental feature that inserts callbacks for data reaching a function,// either via function arguments and loads.
...
// Controls whether the pass tracks the control flow of select instructions.static cl::opt<bool> ClTrackSelectControlFlow(
...
struct TransformedFunction {
  ...
  /// position.  Argument positions are zero-indexed.  If the transformation  /// from F to F' made the first argument of F into the third argument of F',
...
transformFunctionAttributes(const TransformedFunction &TransformedFunction,
  ...
  for (unsigned I = 0, IE = TransformedFunction.ArgumentIndexMapping.size();       I < IE; ++I) {
    unsigned TransformedIndex = TransformedFunction.ArgumentIndexMapping[I];    ArgumentAttributes[TransformedIndex] = CallSiteAttrs.getParamAttrs(I);
  }
...
class DataFlowSanitizer : public PassInfoMixin<DataFlowSanitizer>{
  ...
  enum WrapperKind {
    /// This function is present in an uninstrumented form but we don't know    /// how it should be handled.  Print a warning and call the function anyway.
...
public:
  explicit DataFlowSanitizer(const std::vector<std::string> &ABIListFiles) 
  : ABIListFiles(ABIListFiles) {
    std::vector<std::string> AllABIListFiles(std::move(this->ABIListFiles));    llvm::append_range(AllABIListFiles, ClABIListFiles);
    // 创建特例列表
...
struct DFSanFunction {
  ...
  /// When Zero is nullptr, it uses ZeroPrimitiveShadow. Otherwise it can be  /// zeros with other bitwidths.
...
bool LibAtomicFunction(const Function &F) {
  ...
  // The DFSan pass would need to be refactored to be function pass oriented  // (like MSan is) in order to fit together nicely with TargetLibraryInfo.
  // We need this check to prevent them from being instrumented, or wrapped.  // Match on name and number of arguments.
...
void DFSanFunction::addReachesFunctionCallbacksIfEnabled(IRBuilder<> &IRB,
  ...
  if (DFS.shouldTrackOrigins()) {
    ...
    args = { DataShadow, DataOrigin, FilePathPtr, CILine, FunctionNamePtr };    CB = IRB.CreateCall(DFS.DFSanReachesFunctionCallbackOriginFn, args);
  } else {
...
bool DataFlowSanitizer::initializeModule(Module &M) {
  ...
  DFSanMaybeStoreOriginFnTy = FunctionType::get(
      Type::getVoidTy(*Ctx), DFSanMaybeStoreOriginArgs, /*isVarArg=*/false);  Type *DFSanMemOriginTransferArgs[3] = {Int8Ptr, Int8Ptr, IntptrTy};
  ...
  DFSanMemTransferCallbackFnTy =
      FunctionType::get(Type::getVoidTy(*Ctx), DFSanMemTransferCallbackArgs,                        /*isVarArg=*/false);
...
void DataFlowSanitizer::addGlobalNameSuffix(GlobalValue *GV) {
  ...
  // corrupting asm which happens to contain the symbol name as a substring.  // Note that the substitution for .symver assumes that the versioned symbol
...
void DataFlowSanitizer::buildExternWeakCheckIfNeeded(IRBuilder<> &IRB,
  ...
  // but replacing with a known-to-not-be-null wrapper can break this check.  // When replacing uses of the extern weak function with the wrapper we try  // to avoid replacing uses in conditionals, but this is not perfect.
...
void DataFlowSanitizer::initializeCallbackFunctions(Module &M) {
  ...
  }
  {
    ...
    DFSanReachesFunctionCallbackOriginFn =
        Mod->getOrInsertFunction("__dfsan_reaches_function_callback_origin",                                 DFSanReachesFunctionCallbackOriginFnTy, AL);
  }
}

bool DataFlowSanitizer::runImpl(
    Module &M, llvm::function_ref<TargetLibraryInfo &(Function &)> GetTLI) {  initializeModule(M);
  ...
  for (std::vector<Function *>::iterator FI = FnsToInstrument.begin(),
    ...
    if (isInstrumented(&F)) {
      ...
      // Build a wrapper function for F.  The wrapper simply calls F, and is      // added to FnsToInstrument so that any instrumentation according to its
      ...
      *FI = NewF;
      ...
      if (!F.isDeclaration()) {
        ...
        // uninstrumented function and hence needs to keep the original ABI.        // But any functions it may call need to use the instrumented ABI, so
        // we instrument it in a mode which preserves the original ABI.
        FnsWithNativeABI.insert(&F);

        // This code needs to rebuild the iterators, as they may be invalidated
        // by the push_back, taking care that the new range does not include        // any functions added by this code.
  ...
  for (Function *F : FnsToInstrument) {
    ...
    // DFSanVisitor may create new basic blocks, which confuses df_iterator.    // Build a copy of the list before iterating over it.
    ...
    for (BasicBlock *BB : BBList) {
      ...
      while (true) {
        ...
        // instruction's next pointer and moving the next instruction to the        // tail block from which we should continue.
    ...
    // yet).  To make our life easier, do this work in a pass after the main    // instrumentation.
    if (ClDebugNonzeroLabels) {
      for (Value *V : DFSF.NonZeroChecks) {
        ...
        Value *Ne =
            IRB.CreateICmpNE(PrimitiveShadow, DFSF.DFS.ZeroPrimitiveShadow);        BranchInst *BI = cast<BranchInst>(SplitBlockAndInsertIfThen(
...
DataFlowSanitizer::getShadowOriginAddress(Value *Addr, Align InstAlignment,
  ...
  if (shouldTrackOrigins()) {
    ...
    if (OriginBase != 0)
      OriginLong =
          IRB.CreateAdd(OriginLong, ConstantInt::get(IntptrTy, OriginBase));    const Align Alignment = llvm::assumeAligned(InstAlignment.value());
...
bool DFSanFunction::isLookupTableConstant(Value *P) {
  if (GlobalVariable *GV = dyn_cast<GlobalVariable>(P->stripPointerCasts()))    if (GV->isConstant() && GV->hasName())
...
std::pair<Value *, Value *> DFSanFunction::loadShadowFast(
  ...
  for (uint64_t ByteOfs = BytesPerWideShadow; ByteOfs < Size;
    ...
    if (ShouldTrackOrigins) {
      Value *NextOrigin = DFS.loadNextOrigin(Pos, OriginAlign, &OriginAddr);      AppendWideShadowAndOrigin(NextWideShadow, NextOrigin);
    }
  }
...
std::pair<Value *, Value *> DFSanFunction::loadShadowOriginSansLoadTracking(    Value *Addr, uint64_t Size, Align InstAlignment, Instruction *Pos) {
  const bool ShouldTrackOrigins = DFS.shouldTrackOrigins();
  ...
  for (const Value *Obj : Objs) {
    if (isa<Function>(Obj) || isa<BlockAddress>(Obj))
      continue;
    if (isa<GlobalVariable>(Obj) && cast<GlobalVariable>(Obj)->isConstant())      continue;
  ...
  switch (Size) {
  case 1: {
    LoadInst *LI = new LoadInst(DFS.PrimitiveShadowTy, ShadowAddr, "", Pos);    LI->setAlignment(ShadowAlign);
    return {LI, Origin};
  }
  // case 2: {
  //   IRBuilder<> IRB(Pos);
  //   Value *ShadowAddr1 = IRB.CreateGEP(DFS.PrimitiveShadowTy, ShadowAddr,  //                                      ConstantInt::get(DFS.IntptrTy, 1));
  //   Value *Load =
...
void DFSanVisitor::visitLoadInst(LoadInst &LI) {
  ...
  // atomic application loads and stores to ensure happen-before order; load  // shadow data after application data; store zero shadow data before
...
void DFSanFunction::paintOrigin(IRBuilder<> &IRB, Value *Origin,
                                ...
                                uint64_t StoreOriginSize, Align Alignment) {  const unsigned OriginSize = DataFlowSanitizer::OriginWidthBytes;
  const DataLayout &DL = F->getParent()->getDataLayout();
...
void DFSanFunction::storeOrigin(Instruction *Pos, Value *Addr, uint64_t Size,
  ...
  if (shouldInstrumentWithCall()) {
    ...
    paintOrigin(IRBNew, updateOrigin(Origin, IRBNew), StoreOriginAddr, Size,                OriginAlignment);
    ++NumOriginStores;
  }
}
...
void DFSanVisitor::visitLandingPadInst(LandingPadInst &LPI) {
  ...
  //
  // For the C++ exceptions, if a value is thrown, this value will be stored  // in a memory location provided by __cxa_allocate_exception(...) (on the
  // throw side) or  __cxa_begin_catch(...) (on the catch side).
  // This memory will have a shadow, so with the loads and stores we will be  // able to propagate labels on data thrown through exceptions, without any  // special handling of the LandingPadInst.
  //
...
void DFSanVisitor::visitAllocaInst(AllocaInst &I) {
  ...
  if (AllLoadsStores) {
    IRBuilder<> IRB(&I);
    DFSF.AllocaShadowMap[&I] = IRB.CreateAlloca(DFSF.DFS.PrimitiveShadowTy);    if (DFSF.DFS.shouldTrackOrigins()) {
...
void DFSanVisitor::visitMemTransferInst(MemTransferInst &I) {
  ...
  MTI->setDestAlignment(DFSF.getShadowAlign(I.getDestAlign().valueOrOne()));  MTI->setSourceAlignment(DFSF.getShadowAlign(I.getSourceAlign().valueOrOne()));
  if (ClEventCallbacks) {
...
bool DFSanVisitor::visitWrappedCallBase(Function &F, CallBase &CB) {
  ...
  case DataFlowSanitizer::WK_Custom:
    ...
    if (Function *CustomFn = dyn_cast<Function>(CustomF.getCallee())) {
      ...
      // Custom functions returning non-void will write to the return label.      if (!FT->getReturnType()->isVoidTy()) {
...
PreservedAnalyses DataFlowSanitizer::run(Module &M,
                                             ModuleAnalysisManager &AM) {
  /*
  if (!ABIList) {
    // Initialize the SpecialCaseList and other settings here.
    std::vector<std::string> AllABIListFiles(ABIListFiles);
    append_range(AllABIListFiles, ClABIListFiles);
    ABIList = SpecialCaseList::createOrDie(AllABIListFiles, *vfs::getRealFileSystem());
  ...
  */
  ...
  if (!DataFlowSanitizer(ClABIListFiles).runImpl(M, GetTLI))
...
extern "C" PassPluginLibraryInfo LLVM_ATTRIBUTE_WEAK llvmGetPassPluginInfo() {
  return {
    LLVM_PLUGIN_API_VERSION, "DFSanPass", LLVM_VERSION_STRING,
    [](PassBuilder &PB) {
      PB.registerPipelineParsingCallback(
        [](StringRef Name, ModulePassManager &MPM,
           ArrayRef<PassBuilder::PipelineElement>) {
          if (Name == "dfsan-pass") {
            MPM.addPass(DataFlowSanitizer(ClABIListFiles));
            return true;
          }
          return false;
        });
