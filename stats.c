...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  static u64 prev_interest, prev_increase;

  if (prev_qp == queued_paths && prev_pf == pending_favored && 
      prev_pnf == pending_not_fuzzed && prev_ce == current_entry &&
      prev_qc == queue_cycle && prev_uc == unique_crashes &&
      prev_uh == unique_hangs && prev_md == max_depth && 
      prev_interest == interest_mutation && prev_increase == increase_mutation) return;
  ...
  prev_md  = max_depth;
  prev_interest = interest_mutation;
  prev_increase = increase_mutation;
  ...
  fprintf(plot_file,
          ...
          unique_hangs, max_depth, eps, total_mutation, interest_mutation, increase_mutation); /* ignore errors */
...
#define SP20 SP10 SP10
  ...
  /*chunkfuzzer start*/
  sprintf(tmp, "%s/%s, %s/%s", DI(stage_finds[STAGE_STRUCT_HAVOC]),
          DI(stage_cycles[STAGE_STRUCT_HAVOC]), DI(stage_finds[STAGE_STRUCT_SPLICE]),
          DI(stage_cycles[STAGE_STRUCT_SPLICE]));
  SAYF(bV bSTOP "struct havoc : " cRST "%-37s " bSTG bVR bH20 bH2 bH2 bRB "\n", tmp);
  
  sprintf(tmp, "%s/%s", DI(stage_finds[STAGE_STRUCT_DESCRIB]),
          DI(stage_cycles[STAGE_STRUCT_DESCRIB]));
  SAYF(bV bSTOP "struct desc  : " cRST "%-37s " bSTG bV "\n", tmp);

  sprintf(tmp, "%s/%s", DI(stage_finds[STAGE_STRUCT_AWARE]),
          DI(stage_cycles[STAGE_STRUCT_AWARE]));
  SAYF(bV bSTOP "struct aware : " cRST "%-37s " bSTG bV "\n", tmp);
  /*chunkfuzzer end*/
  ...
  SAYF(bV bSTOP "        trim : " cRST "%-37s " bSTG bV
