...
static void dump_hex(u8* buf, u32 len, u8* b_data) {
  ...
  u8* out_file = alloc_printf("%s.json", in_file);

  cJSON* root = cJSON_CreateObject();

  u32 node_num = 0;
  // cJSON_AddNumberToObject(root, "start", 0);
  ...
  for (i = 0; i < len; i++) {
    ...
    cJSON* new_node = cJSON_CreateObject();
    cJSON_AddNumberToObject(new_node, "start", start);
    cJSON_AddNumberToObject(new_node, "end", end);

    char* type = NULL;
    switch (rtype) {

      case RESP_NONE:     type = alloc_printf("none"); break;
      case RESP_MINOR:    type = alloc_printf("none"); break;
      case RESP_VARIABLE: type = alloc_printf("critical stream"); break;
      case RESP_FIXED:    type = alloc_printf("magic value"); break;
      case RESP_LEN:      type = alloc_printf("length"); break;
      case RESP_CKSUM:    type = alloc_printf("cksum or magic int"); break;
      case RESP_SUSPECT:  type = alloc_printf("checksummed block"); break;

    }
    cJSON_AddStringToObject(new_node, "type", type);


    char* id = alloc_printf("%d", node_num);
    cJSON_AddItemToObject(root, id, new_node);
    ++node_num;
  }

  u8* out_stream = cJSON_Print(root);

  write_to_file(out_file, out_stream, strlen(out_stream));
